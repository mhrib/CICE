...
module bench_v2
  ...
  subroutine stepu_last(NA_len, rhow, &
                    lb,ub,Cw,aiu,uocn,vocn,forcex,forcey,umassdti,fm,uarear,Tbu, &
                    strocnx,strocny,strintx,strinty,taubx,tauby,                 &
                    uvel_init,vvel_init,uvel,vvel,                               &
                    str1,str2,str3,str4,str5,str6,str7,str8, nw,sw,se,skipme)
    !- modules -------------------------------------------------------------------
    use ice_kinds_mod
...
!    module procedure convert_2d_1d_v0
!    module procedure convert_2d_1d_v1
  ...
  real (kind=dbl_kind), dimension(:), allocatable ::               &
    cdn_ocn,aiu,uocn,vocn,waterx,watery,forcex,forcey,Tbu,tarear,  &
...
!v1      dxhy(1:na),dyhx(1:na),cyp(1:na),cxp(1:na),cym(1:na),cxm(1:na),tinyarea(1:na),&
      ...
      waterx(1:nb),watery(1:nb),forcex(1:nb),forcey(1:nb),Tbu(1:nb),      &
  ...
  subroutine dealloc1d
    ...
    deallocate(                                   &
      ...
      waterx,watery,forcex,forcey,Tbu,            &
      umassdti,fm,uarear,                         &
...
!v1       I_dxhy,I_dyhx,I_cyp,I_cxp,I_cym,I_cxm,I_tinyarea,                                      &
       ...
       I_cdn_ocn,I_aiu,I_uocn,I_vocn,I_waterx,I_watery,I_forcex,I_forcey,I_Tbu,               &
...
!v1       G_dxhy,G_dyhx,G_cyp,G_cxp,G_cym,G_cxm,G_tinyarea,                                      &
       G_cdn_ocn,G_aiu,G_uocn,G_vocn,G_waterx,G_watery,G_forcex,G_forcey,G_Tbu,               &
...
!MHRI     integer(kind=int_kind) :: iiw,ii,jj !,masku,maskt  MHRI
!MHRI do iw=1,navel
!MHRI   if (halo_parent(iw)>0) then
!MHRI     iiw=halo_parent(iw)
!MHRI     j=int((indij(iiw)-1)/(nx))+1
!MHRI     i=indij(iiw)-(j-1)*nx
!MHRI     ii=i
!MHRI     jj=j
!MHRI     j=int((indij(iw)-1)/(nx))+1
!MHRI     i=indij(iw)-(j-1)*nx
!MHRI     write(*,*)iw,i,j,iiw,ii,jj
!MHRI   endif
!MHRI enddo
